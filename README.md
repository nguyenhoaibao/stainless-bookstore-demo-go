...
```sh
go get -u 'github.com/nguyenhoaibao/stainless-bookstore-demo-go@v0.0.1-alpha.0'
```
...
```go
params := FooParams{
	Name: bookstore2.F("hello"),

	// Explicitly send `"description": null`
	Description: bookstore2.Null[string](),

	Point: bookstore2.F(bookstore2.Point{
		X: bookstore2.Int(0),
		Y: bookstore2.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: bookstore2.Raw[int64](0.01), // sends a float
	}),
}
```
...
`*bookstore2.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/echo": 400 Bad Request { ... }
}
```
...
We also provide a helper `bookstore2.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   bookstore2.F("id_xxxx"),
    Data: bookstore2.F(FooNewParamsData{
